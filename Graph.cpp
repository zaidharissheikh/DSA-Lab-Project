...
void Graph::addUserProfile(const std::string& username, UserProfile* user) {
    // Find the vertex index corresponding to the username
    for (int i = 0; i < vertices; ++i) {
        if (usernames[i] == username) {
            adjMatrix[i] = user;  // Associate the username with the UserProfile
            return;
        }
    }
}

UserProfile* Graph::getUserProfile(const std::string& username) const {
    for (int i = 0; i < vertices; ++i) {
        if (usernames[i] == username) {
