...
void UserProfile::addFriendRequest(const string& sender) {
    ...
    friendRequestQueue.enqueue(1);  // You can add an actual identifier here
}

// Function to send a message to a friend
void UserProfile::sendMessage(const string& receiver, const string& message) {
    MessageNode* newMessage = new MessageNode(receiver, message);
    newMessage->next = messages;
    messages = newMessage;

    // Optionally, add the message to the message stack
    messageStack.push(1);  // You can replace 1 with an actual message identifier
}
...
void UserProfile::addNotification(const string& message) {
    ...
    // Optionally, add the notification to the notification queue
    notificationQueue.enqueue(1);  // You can add an actual notification identifier
}
...
bool UserProfile::addUser(const string& username, const string& password) {
    ...
    // Create a new user profile and add to the global BST and hash table
